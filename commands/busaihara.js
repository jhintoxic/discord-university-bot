...
module.exports = {
  data: new SlashCommandBuilder().setName('bus-aihara').setDescription('相原駅発のスクールバス、次の時刻を調べます'),
  ...
  async execute(interaction) {
    ...
    const { times, finished } = bus.getUpcoming('aihara', now, 3);

    if (finished) {
      await interaction.reply(`本日(${bus.DAY_LABELS[dayType]})の相原駅発バスは、すべて発車済みです。`);
      return;
    }
